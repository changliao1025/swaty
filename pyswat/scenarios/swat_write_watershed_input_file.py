...
def swat_write_watershed_input_file(oModel_in):
    """
    ...
    for iVariable in range(nvariable):
        ...
        if sVariable in aBSN:
            if( aParameter_table[0] is None  ):
                aParameter_table[0] = sVariable
            else:
                aParameter_table[0].append(sVariable) 
            pass
        else:
            pass
    ...
    for iFile_type in range(0, nFile_type):
        ...
        if( iFlag == 1):
            sFilename = 'basins.bsn'
            sFilename_watershed = sWorkspace_source_case + 'TxtInOut' + slash + sFilename 
            ...
            while sLine:
                ...
                for i in range(0, aParameter_count[iFile_type]):
                    if 'sftmp' in sLine.lower() : 
                        dummy = 'SFTMP' 
                        dummy1 = np.array(aParameter_index[0])
                        dummy2 = np.array(aParameter_user[0])
                        dummy_index1 = np.where(dummy2 == 'SFTMP')
